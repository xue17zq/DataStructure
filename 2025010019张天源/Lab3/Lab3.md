...
- [x] A. 先 `data[2] = data[3]`，再 `data[3] = data[4]`，最后 `data[2] = 25`
- [ ] B. 先 `data[3] = data[2]`，再 `data[4] = data[3]`，最后 `data[2] = 25`
- [ ] C. 先 `data[4] = data[3]`，再 `data[3] = data[2]`，最后 `data[2] = 25`
...
> insertAt插入从后方前移，防止还没移动就覆盖尚未搬运的有效数据，removeAt删除从前向后移。方向写反，会把还没有移动的元素覆盖，造成数据丢失。
