...
| C（Lab3 / 课堂） | C++（本次作业）        |
| :--- |:-----------------------|
| `SeqList list; list.size = 0;` | InVector list;         |
| `insertAt(&list, 2, 25)` | list.insert(2,25);     |
| `removeAt(&list, 1, &removed)` | list.remove(1,remove); |
| 函数体内的 `list->data[i]` | list.data[i];          |
...
> 在此填写你的回答（不少于 20 字）。
size设置为private之后，外部代码无法直接修改size的值，只能通过类提供的insert、remove等成员函数修改，所有对size的改动都会经过函数内部的边界校验，保证size永远合法，避免外部随意篡改该造成数据不一致。
---
